...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        """
        seed = random.random()
        if (seed <= self.getMaxBirthProb() * (1 - popDensity)):
...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in virusesCopy:
            try:
                child = virus.reproduce(density)
                self.getViruses().append(child)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        seed = random.random()
        if (seed < self.maxBirthProb * (1 - popDensity)):
            offspringResistants = {}
            for d in self.getResistances():
                mutSeed = random.random()
                if self.isResistantTo(d):
                    if (mutSeed < self.getMutProb()):
                        offspringResistants[d] = False
                    else:
                        offspringResistants[d] = True
                else:
                    if (mutSeed < self.getMutProb()):
                        offspringResistants[d] = True
                    else:
                        offspringResistants[d] = False
            offspring = ResistantVirus(self.getMaxBirthProb(),
                                       self.getClearProb(),
                                       offspringResistants,
                                       self.getMutProb())
            return offspring
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in virusesCopy:
            try:
                child = virus.reproduce(density, self.getPrescriptions())
                self.getViruses().append(child)
            except NoChildException:
                continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    pylab.plot(range(timestep), population)

    pylab.plot(range(timestep), rsp)
