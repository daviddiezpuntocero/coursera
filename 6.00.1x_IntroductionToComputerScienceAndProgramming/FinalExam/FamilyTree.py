...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        def getRootChain(node):
            rootChain = []
            while node.get_parent() != None:
                rootChain.append(node.get_parent())
                node = node.get_parent()
            return rootChain

        def getDiffToCommonAncestor(chain1, chain2):
        ...
        ma = self.names_to_nodes[a]
        mb = self.names_to_nodes[b]
        aRootChain = getRootChain(ma)
        bRootChain = getRootChain(mb)

        if ma == mb or ma.is_child(mb) or ma.is_parent(mb) or ma in bRootChain or mb in aRootChain:
            diffToCommonAncestor = -1
        elif ma.get_parent() == mb.get_parent():
            diffToCommonAncestor = 0
        else:
            diffToCommonAncestor = getDiffToCommonAncestor(aRootChain, bRootChain)
        diffToRoot = abs(len(aRootChain) - len(bRootChain))
        #print diffToCommonAncestor, diffToRoot
        return diffToCommonAncestor, diffToRoot
...
t, r = f.cousin("c", "g")
print "'c' is a", words[t], "cousin", r, "removed from 'g'"

t, r = f.cousin("j", "b")
print "'j' is a", words[t], "cousin", r, "removed from 'b'"
